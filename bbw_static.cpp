...
int main(int argc, char *argv[])
{
  using namespace Eigen;
  using namespace std;
  igl::readMESH(argc>=1 ? argv[1]: "data/hand.mesh",V,T,F);
  U=V;
  igl::readTGF(argc>=2 ? argv[2]:"data/hand.tgf",C,BE);
  cout << "V,T,F:" << V.rows() << ", " << T.rows() << ", " << F.rows() << endl;
  // retrieve parents for forward kinematics
  igl::directed_edge_parents(BE,P);
  cout << "C, BE:" << C.rows() << ", " << BE.rows() << endl;

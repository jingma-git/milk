    ...
    min_quad_with_fixed_data<T> &data)
{
  ...
  // parameter
  // cout << "define Auu's positiveness and symetriness...." << endl;
  data.Auu_pd = pd;
  if (data.Auu_pd)
  {
  ...
  // cout << "positive: " << data.Auu_pd << ", symmetry: " << data.Auu_sym << endl;
  ...
  if (neq > 0)
  {
    ...
    nc = data.AeqTQR.rank();
    // cout << "independent rows in Aeq: " << nc << endl;
    assert(nc <= neq &&
  ...
  if (data.Aeq_li)
  {
    ...
    // cout << "compute RHS builders...." << endl;
    if (kr > 0)
    {
    ...
    {
      // cout << "positive definite with #Aeq=0, use LLT" << endl;
      data.Auu = Auu; // Add by Jing Ma for Debug purpose
#ifdef MIN_QUAD_WITH_FIXED_CPP_DEBUG
      cout << "    llt" << endl;
#endif
    ...
    else
    {
      ...
      {
        // cout << "not positive definite or #Aeq=" << Aeq.rows() << ", use LU" << endl;
#ifdef MIN_QUAD_WITH_FIXED_CPP_DEBUG
        cout << "    lu" << endl;
#endif
        // Resort to LU
  ...
  {
    cout << "Aeq is not linear indepdent..." << endl;
#ifdef MIN_QUAD_WITH_FIXED_CPP_DEBUG
    cout << "    Aeq_li=false" << endl;
#endif
    data.neq = neq;
    const int nu = data.unknown.size();
    cout << "#unknows: " << nu << endl;
    cout << "#eq: " << neq << endl;
    cout << "Aeq rank: " << nc << endl;
    //cout << "    matrixR" << endl;
    ...
    Eigen::PlainObjectBase<Derivedsol> &sol)
{
  ...
  // Set known values
  // cout << "set know values..." << endl;
  for (int i = 0; i < kr; i++)
  {
    for (int j = 0; j < cols; j++)
    {
      Z(data.known(i), j) = Y(i, j);
      // cout << "boundary indice " << data.known(i) << ", boundary value: " << Y(i, j) << endl;
    }
  }
  ...
  if (data.Aeq_li)
  {
    ...
    // cout << "built right hand side..." << endl;
    MatrixXT BBequlcols;
